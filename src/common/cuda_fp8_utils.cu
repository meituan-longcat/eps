...
#define defineinvokeQuantizeMatrix(type_out, type_in, mode)    \
  template void invokeQuantizeMatrix<type_out, type_in, mode>( \
      type_out * output,                                       \
      float const* input_scale,                                \
      type_in const* input,                                    \
      uint32_t size,                                           \
...
template void invokeFakeQuantize<__nv_bfloat16, __nv_bfloat16, __nv_fp8_e4m3>(
    __nv_bfloat16* dst,
...
template <typename T_W>
void invokeComputeFP8QuantizeScale(
    float* quant_ptr,
    const T_W* weights,
    const int k,
    const int n,
